...
//             submit('submit', _("Save"), true, '', false, 'save');
    ...
    function items()
    {
        ...
        box_footer_start();
        echo '<a title="Add new item" class="ajaxsubmit btn green btn_right" href="' . site_url('opening/'.$this->type.'/add') . '"><i class="material-icons">add</i> Add </a>';
        box_footer_end();
    }
    ...
    function row_actions($row, $cell = NULL)
    {
        ...
        $html = '<a class="ajaxsubmit button" href="' . site_url($ci->uri->uri_string()) . '/view?edit=' . $row['id'] . '" >' . ((abs($allocation) > 0) ? '<i class="fa fa-eye"></i>' : '<i class="fa fa-edit"></i>') . '</a>';

        if (abs($allocation) == 0) {
            $html .= ' <a class="ajaxsubmit button" href="' . site_url($ci->uri->uri_string()) . '?remove=' . $row['id'] . '" ><i class="fa fa-remove"></i></a>';
        }
