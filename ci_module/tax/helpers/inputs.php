...
function input_multitaxes($label, $name, $selected_ids=array()){
    ...
    if( !empty($items) ){
        ...
        foreach ($items AS $tax){
            $idname = "tax".$tax->id;
            $html.= '<div>';
            $html.= '<input type="checkbox" '.( ((is_array($val) && in_array($tax->id, $val)) OR ($val==$tax->id)) ? 'checked' : null ).' id="'.$idname.'" name="'.$name.'['.$tax->id.']" value="'.$tax->id.'" />';
            $html.= '<label for="'.$idname.'">'.$tax->title.'</label>';
            $html.= '</div>';
        }
    }
    $html.= '</div>';
