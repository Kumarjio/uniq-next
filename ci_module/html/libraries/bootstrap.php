...
//         var_dump($attributes);
    ...
    function box_start($title = NULL, $icon = NULL, $new_row=false, $box_id=NULL)
    {
        ...
        if (strlen($title) > 0) {
            ...
            $box_title .= '';
            $box_title .= '<span class="caption-subject bold uppercase"><span synlang="syncard-language">' . $title . '</span></span>';
            $box_title .= '</div>';

            $html.= '<div class="portlet-title"><span synlang="syncard-language">' . $box_title . '</span></div>';
        }
 ...
 */
    ...
    function box_footer_start($class = NULL, $attributes = NULL,$show_back=true)
    {
        ...
        $attributes['class'] = 'form-actions clearfix col s12';
        if (is_string($attributes)) {
            $attributes = _attributes_str2array($attributes);
        }
        echo "<div " . _parse_attributes($attributes) . ">";
