...
function start_table($class = false, $attributes = "", $padding = '2', $spacing = '0')
{
    ...
    if (! strpos($attributes['class'], 'table-striped')) {
        $attributes['class'] .= ' responsive-table table';
    }

    $attributes['class'] .= ' table-responsive';
    echo "<table id='sync-table' " . _parse_attributes($attributes) . ">\n";
}
...
function labelheader_cell($label, $params = "")
{
    ...
    echo "<th " . _parse_attributes($params) . " ><span synlang='syncard-language'>".($label)."</span></th>\n";
}
...
function display_db_pager(&$pager)
{
    ...
    $option= array("gl_view");

    div_start("_{$pager->name}_span" , $trigger = null, $non_ajax = false, $attributes = 'class="table-box"');
    $headers = array();
...
//             $attributes['label'] = $header_label;
            if(isset($col['fun'])){
                  ...
                  //operation button
                  $dumb = explode("_",$col['fun']);
                  if(isset($dumb[1]) && $col['fun'] != "view_link" && ($dumb[1] == "link" || in_array($col['fun'],$option))){
                    $header_label = "break-fun";
                  }
            }

            //operation button
            if($header_label != "break-fun")
