...
//         $ci = get_instance();
    ...
    public function invoice_print(){
        ...
        $gst = $_SESSION['SysPrefs']->prefs['gst_no'];
        if($gst == NULL){
            $this->rep = $this->report->front_report(_('INVOICE'),$this->invoice_report_table,ST_SALESINVOICE);
        }else{
    ...
    private function pdf_print($tran=NULL){
        ...
        if( empty($items) ){
		    ...
		    $tax = tax_calculator($detail->tax_type_id,$Net,$tran->tax_included);			
...
/*                $this->rep->Font('bold');
                ...
                $this->rep->TextCol(7, 8,   number_total( $null ));*/
            }else{
                $this->rep->Font('bold');
                $this->rep->TextCol(5, 7,   _('TOTAL INCL. GST'));
