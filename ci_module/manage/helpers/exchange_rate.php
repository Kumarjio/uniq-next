...
function get_extern_rate($curr_b, $provider = 'GOOGLE', $date)
{
    ...
    if ($provider == 'ECB') {
        ...
        $filename = "/d/quotes.csv?s={$curr_b}{$curr_a}=X&f=sl1d1t1ba&e=.csv"; // new URL's for YAHOO
        $site = "download.finance.yahoo.com";
        // $filename = "/q?s={$curr_a}{$curr_b}=X"; // Let old code be here for a while, Joe.
        // $site = "finance.yahoo.com";
    } elseif ($provider == 'GOOGLE') {
        $filename = "/finance/converter?a=1&from={$curr_b}&to={$curr_a}&meta=ei%3D-VIJWoH_KdOouASMjJKABQ";
        $site = "www.google.com";
