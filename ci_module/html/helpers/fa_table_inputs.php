...
function inactive_control_cell($id, $value, $table, $key)
{
    ...
    if (check_value('show_inactive')) {
        ...
        echo '<td align="center">' . checkbox(null, $name, $value, true, ''). '<label for="checkboxs_'.$name.'"></label>' . hidden("LInact[$id]", $value, false) . '</td>';
    }
}
//
...
{
    if ($label != null)
        echo "<td $params><span synlang='syncard-language'>$label</span></td>\n";
...
function qty_cells($label, $name, $init = null, $params = null, $post_label = null, $dec = null)
{
    ...
    amount_cells_ex($label, $name, 3, 3, $init, $params, $post_label, $dec);
}
