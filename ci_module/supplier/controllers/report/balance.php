...
//         $this->customer_trans_model = module_model_load('customer_trans','sales');
    ...
    function balance_print(){
        ...
        while ($myrow = db_fetch($result)){
            ...
            $rep->TextCol(6,7,  number_total(abs($line_total['balance'])) );

            $rep->NewLine(1, 2);

            $trans_items = $this->trans_model->get_transactions($myrow['supplier_id'], $from, $to);
    
            // condition for empty transaction //

            if(count($trans_items) == 0){

                $grand_total['balance'] += $line_total['balance'];

            }

            // condition for empty transaction end //
                ...
                foreach ($this->balance_report_table AS $k=>$val){
                    ...
                    switch ($k){
                        ...
                        case 'debit_base':
                            ...
                            break;
                        case 'balance':
                            $txt = number_total(abs($line_total[$k]));
                             break;
                        default:
                            $txt = $trans[$k]; break;
                    }
            ...
            $rep->TextNum(10, 11, abs($line_total['balance']));
        ...
        $rep->TextNum(10, 11, abs($grand_total['balance']));
